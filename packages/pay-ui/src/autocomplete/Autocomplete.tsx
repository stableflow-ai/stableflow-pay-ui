...
function nextEnabledIndex(options: readonly AutocompleteOption[], from: number, direction: 1 | -1): number {
...
export function Autocomplete(props: AutocompleteProps) {
  ...
  useEffect(() => {
    ...
    const match = value != null ? optionsRef.current.findIndex((option) => option.value === value) : -1;
    setActive(match);
  ...
  return (
    <div className={cn("relative min-w-0", className)}>
      <div
        ...
        aria-expanded={open}
        aria-activedescendant={open && activeIndex >= 0 ? `autocomplete-option-${activeIndex}` : undefined}
      >
        {children}
      </div>
      {open && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              className={cn(
                "max-h-60 w-max overflow-x-hidden overflow-y-auto rounded-[12px] border border-[#E0E0E0] bg-[#FDFDFD] py-1 text-base font-medium leading-normal text-black shadow-[0_0_20px_0_rgba(0,0,0,0.06)]",
                fontSans,
                panelClassName,
              )}
            >
