...
function useTableContext() {
  ...
  if (!context) throw new Error("Table compound components must be used within Table.");
  return context;
}
...
export function Table(props: TableProps) {
  const { columns, className, scrollClassName, toolbar, footer, children, style, ...restProps } = props;

  return (
    <TableContext.Provider value={{ columns }}>
      <Card className={cn("flex min-h-0 flex-col overflow-hidden p-5", className)} style={style} {...restProps}>
        {toolbar ? <div className="shrink-0">{toolbar}</div> : null}
        <div className={cn("min-h-0 flex-1 overflow-auto [scrollbar-gutter:stable]", scrollClassName)}>
...
export function TableHeader(props: TableHeaderProps) {
  ...
  return (
    <div
      className={cn(
        "sticky top-0 z-10 grid w-full min-w-min border-b border-black/10 bg-[#FDFDFD] text-sm font-medium text-[#aaa] capitalize",
        fontSans,
        className,
      )}
...
export function TableRow(props: TableRowProps) {
  ...
  return (
    <div
      className={cn(
        "grid w-full min-w-min border-b border-black/10 text-sm font-medium text-black last:border-b-0",
        fontSans,
        className,
      )}
      style={{ ...columnsStyle(columns), ...style }}
